...
public class SimpleMeshCombineEditor extends Editor {
    ...
    override function OnInspectorGUI () {
		...
		if(!target.combined){
        	...
        	//	GUILayout.Label("Material count: " + target.combined.renderer.sharedMaterials.Lenght,infoStyle);
        	}
        }
        ...
        if(target.combined && !target._savedPrefab){       	
        	...
        	if(GUILayout.Button("Save Mesh", buttonStyle2)) {
        		var n:String = target.meshName+"_"+target.gameObject.name;
        		Debug.Log(n);
        		if(System.IO.Directory.Exists("Assets/Simple Mesh Combine/Saved Meshes/")){
        		if(!System.IO.File.Exists("Assets/Simple Mesh Combine/Saved Meshes/"+n+".asset")){     	
