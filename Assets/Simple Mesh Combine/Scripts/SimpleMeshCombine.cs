...
public class SimpleMeshCombine:MonoBehaviour {
	...
	//Used when checking if this mesh has been saved to prefab (saving the same mesh twice generates error)
	...
	public MeshFilter[] FindEnabledMeshes ()
	{
		...
		for (int i = 0; i < renderers.Length; i++) {//count all the enabled meshrenderers in children
		...
		count = 0;
		for (int ii = 0; ii < renderers.Length; ii++) {//adds all enabled meshes to the array
